...
import com.collage.empowermentstrishakti.Adapter.DiscussionAdapter
import com.collage.empowermentstrishakti.Adapter.ParticipantsAdapter
import com.collage.empowermentstrishakti.Common.SessionManager
import com.collage.empowermentstrishakti.Common.ViewUtils
import com.collage.empowermentstrishakti.Factory.EventViewModelFactory
...
import com.collage.empowermentstrishakti.ui.RegisterViewModel.EventViewModel.ActionResult
...
//    }
    ...
    private fun setupObservers() {
        ...
        // Join event result (Success / Failure / Idle)
        viewModel.joinResult.observe(this) { result ->
            when (result) {
                is EventViewModel.ActionResult.Success -> {
                    AlertDialog.Builder(this)
                        .setTitle("Success")
                    ...
                    // Ask ViewModel to update details LiveData
                    viewModel.markCurrentUserAsParticipant()
                }


                is EventViewModel.ActionResult.Failure -> {
                    Toast.makeText(this, result.message, Toast.LENGTH_SHORT).show()
                }

                EventViewModel.ActionResult.Idle -> {
                    // No action needed
                }
            }
        }

        // Participants list observer
        viewModel.participants.observe(this) { list ->
            if (::participantsAdapter.isInitialized) {
                participantsAdapter.submitList(list ?: emptyList())
            }
        ...
        // Exit event result (Success / Failure / Idle)
        viewModel.exitResult.observe(this) { result ->
            when (result) {
                is EventViewModel.ActionResult.Success -> {
                    Toast.makeText(this, result.message, Toast.LENGTH_SHORT).show()
                    ...
                    if (!isHost) {
                        ...
                        // inform ViewModel to update details LiveData
                        viewModel.markCurrentUserAsNotParticipant()
                    }

                    // if host removed participant, ViewModel already updated LiveData
                }
